<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Payout by plan</h2>
            <a href="{{ route('admin.reports.payout-by-plan', ['export' => 'csv']) }}" class="text-sm text-indigo-600 hover:text-indigo-900">Export CSV</a>
        </div>
    </x-slot>
        ...
        <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200 text-sm">
                    @forelse ($rows as $row)
                        <tr>
                            <td class="px-6 py-4 text-gray-900">{{ $row['plan'] }}</td>
